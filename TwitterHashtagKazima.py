...
wait = WebDriverWait(driver, 20) # Otomatik Bekleme Süreleri İçin. 10 burada maksimum bekleme süresini ifade eder. Eğer 10 saniye ve üstü sürerse TimeoutException hatası döner.
...
for k in range(0, TweetSayisi):
    ...
    if Bilgi[k].split("\n")[5] == " adlı kişiye yanıt olarak":
        Tweet = (Bilgi[k].split("\n")[6])
    sayac += 1
    print(f"{sayac} - {When} - {Account}: {Tweet}")
